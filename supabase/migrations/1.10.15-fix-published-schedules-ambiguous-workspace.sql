-- Fix ambiguous column references in published schedule RPCs from 1.10.14.
-- Run this migration after 1.10.14-published-schedules.sql.
...
using (
  exists (
    ...
    where wm.workspace_id = public.published_schedules.workspace_id
...
create or replace function public.publish_schedule_to_employee_domain(
  p_workspace_id uuid,
  p_week_start date,
...
begin
  ...
  return query
  with upserted as (
    insert into public.published_schedules as ps (
      workspace_id,
      week_start,
      schedule_json,
      published_at,
    ...
    on conflict on constraint published_schedules_workspace_week_unique do update
    ...
    returning
      ps.id as row_id,
      ps.workspace_id as row_workspace_id,
      ps.week_start as row_week_start,
      ps.schedule_json as row_schedule_json,
      ps.published_at as row_published_at,
      ps.updated_at as row_updated_at
  )
  select
    upserted.row_id,
    upserted.row_workspace_id,
    upserted.row_week_start,
    upserted.row_schedule_json,
    upserted.row_published_at,
...
create or replace function public.clear_published_schedule(
  p_workspace_id uuid,
  p_week_start date
)
returns void
...
begin
  ...
  delete from public.published_schedules as ps
  where ps.workspace_id = p_workspace_id
    and ps.week_start = p_week_start;
end;
$$;
...
create or replace function public.get_public_published_schedule(
  p_week_start date,
