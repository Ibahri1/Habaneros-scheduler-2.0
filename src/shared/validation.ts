...
export const workerSchema = z.object({ id: z.string().min(1), employeeCode: z.string().regex(/^\d{4}$/).or(z.literal("")), name: z.string().min(1).max(120), position: z.string().min(1).max(120), role: z.enum(["Crew", "Lead"]), isManager: z.boolean(), skillRating: z.number().int().min(1).max(10), noHourLimits: z.boolean(), maxWeeklyHours: z.number().min(0).max(168), preferredWeeklyHours: z.number().min(0).max(168), maxDays: z.number().int().min(1).max(7), canOpen: z.boolean(), canClose: z.boolean(), active: z.boolean(), notes: z.string(), availability: z.array(daySchema), shiftAvailability: shiftAvailabilityMapSchema, shiftTimes: z.object({ open: shiftTimeSchema, close: shiftTimeSchema }) });
