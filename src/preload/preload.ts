...
const api = {
  ...
  deactivateCloudEmployee: (localWorkerId: string): Promise<CloudResult> => ipcRenderer.invoke("cloud:employees:deactivate", { localWorkerId }),
  resetEmployeeCalendarToken: (localWorkerId: string): Promise<CloudResult & { calendarToken?: string }> => ipcRenderer.invoke("cloud:employees:calendar:reset", { localWorkerId }),
