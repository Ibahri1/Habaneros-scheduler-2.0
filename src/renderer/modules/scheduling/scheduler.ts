...
function canWork(worker: Worker, day: DayName, shiftName: ShiftName, rules: ScheduleRules, stats: AssignmentStats, assignedToday: Set<string>): boolean {
  ...
  if (shiftAvailability === "Unavailable" || (shiftAvailability !== "Both" && shiftAvailability.toLowerCase() !== shiftName)) return false;
  if (assignedToday.has(worker.id)) return false;
  if ((stats.days[worker.id] || 0) >= worker.maxDays) return false;
  return worker.noHourLimits || (stats.hours[worker.id] || 0) + shiftDuration(worker, shiftName) <= worker.maxWeeklyHours;
}
...
function toAssignedWorker(worker: Worker, shiftName: ShiftName, rules: ScheduleRules): AssignedWorker {
  ...
  return { assignmentId: randomUUID(), id: worker.id, name: worker.name, position: worker.position, role: worker.role, isManager: worker.isManager, start, end, timeRange: formatTime(start) + "-" + formatTime(end), durationHours, needsLunch: durationHours >= rules.mealBreakHours };
}
...
function explainNoCandidates(day: DayName, shiftName: ShiftName, workers: Worker[], stats: AssignmentStats, assignedToday: Set<string>): string {
  ...
  if (active.every((worker) => !worker.availability.includes(day) || (!worker.noHourLimits && (stats.hours[worker.id] || 0) + shiftDuration(worker, shiftName) > worker.maxWeeklyHours))) return "Available employees would exceed maximum weekly hours.";
